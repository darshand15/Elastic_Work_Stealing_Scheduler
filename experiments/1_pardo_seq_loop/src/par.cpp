...
int main(int argc, char* argv[])
{
    ...
    parlay::par_do(
      [&]() { ans1 = seq_loop(atoi(argv[1])); },
      [&]() { ans2 = seq_loop(atoi(argv[1]) + 786); }
    );
    ...
    // cout << fixed;
    // cout << "Computed values: " << ans1 << " " << ans2 << "\n";

    // auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    // cout << "Parallel Time Taken: " << duration.count() << " microseconds\n";
    ...
    cout << "Parallel User Time Taken: " << duration_utime << " microseconds\n";
